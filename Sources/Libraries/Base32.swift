...
// credits & slightly modified from ⇝ https://gist.github.com/troupmar/b5bf94be41108a905fcc6a8d09031ced
private func base32Encode(data: Data) -> String {
...
private func base32Encode(data: Data, alphabet: [String]) -> String {
	var result = ""
...
private func base32Decode(data: String, alphabet: [Int], characters: [String]) -> Data? {
	var processingData = ""

	for char in data.uppercased() {
		let string = String(char)

		if characters.contains(string) {
			processingData += string
		}
		else if !characters.contains(string) && string != "=" {
			return nil
		}
	}
	...
	if let base32Data = processingData.data(using: String.Encoding.ascii, allowLossyConversion: false) {
		...
		var bytesInPartialGroup = 0

		switch base32Data.count % 8 {
			case 0: bytesInPartialGroup = 0
			case 2: bytesInPartialGroup = 1
			case 4: bytesInPartialGroup = 2
			case 5: bytesInPartialGroup = 3
			case 7: bytesInPartialGroup = 4
			default: return nil
		}
...
extension String {
	static func base32EncodedString(_ data: Data) -> String {
		return base32Encode(data: data)
	}
}

extension Data {
	static func base32DecodedString(_ data: String) -> Data {
		return base32Decode(data: data) ?? Data()
	}
}
